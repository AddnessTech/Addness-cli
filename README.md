...
- codex 画面上でのトラックパッド/ホイール … ポインタ下の codex / Addness 枠をスクロール
- `F12` … 実行中の codex を終了して戻る / `Esc`・`q` … ペインを閉じる

codex 枠で遡れる量は、軽さとのトレードオフです。長い会話を多めに遡りたい場合は
`ADDNESS_CODEX_EXTRA_SCROLL_ROWS=320 cargo run` のように、codex 用 PTY の余分な行数を増やせます
（デフォルトは 160、最大 1000）。
