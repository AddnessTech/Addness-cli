...
| 13 | AIエージェントチャット / Goal Decompose | 17 | 10 | 4 | 3 |
...
**全体として、ユーザー向け実質エンドポイントのうちCLIが実装済みなのは約6割。** 通知・定期ゴール・ユーザー・組織・メンバー/招待・コメント/Goal Issue/組織チャット・実行タブ/カレンダー・アクティビティログ・ストリーク・個人スペース・検索/診断/紹介/請求書/共有ツリー/メディア・ミーティング（ライブ参加系を除く）・スキル・Codexジョブ・APIキーの各グループは実装済み。残る大きな未着手領域は、外部連携（OAuth導線）・AIスレッド/エージェント・AIエージェントチャット（SSE）・ゴールのAIバックグラウンドタスク。
...
| GET | /api/v2/me/consents/:consentType | `consent get` | 実装済み |
| POST | /api/v2/me/consents | - | 対象外（Clerk認証限定: 法的同意の本人記録のためAPIキー認証を一律403で拒否。`api_consent_route_policy_test.go` で固定化） |
...
スキルCRUD/検索/パフォーマンス/リソースCRUD/改善提案の承認・却下（15本）は **実装済み**（`skill create/list/general/search/get/update/delete/performance/resource/refinement`）。Legacy Tools APIと`tool`コマンドは、認可を欠くexecutor APIの公開停止に伴い廃止した。
...
関連ドキュメント: [`docs/backend-endpoint-inventory.md`](./backend-endpoint-inventory.md)（本表の元になったバックエンドルート棚卸し表）
