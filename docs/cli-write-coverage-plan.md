...
| CLI 実装済み書き込みサブコマンド | 7 |

### CLI 実装済み（書き込み）
- `goal create` / `goal update` (title/status のみ) / `goal delete`
- `comment create`
- `deliverable add` (テキスト or ファイル添付)
...
| update (全フィールド) | PATCH /v2/objectives/:id | 🟡 (title/status のみ) |
