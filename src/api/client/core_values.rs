...
use crate::api::{
    ApiClient, ApiResponse, CoreValuesChatStreamRequest, CoreValuesMessage, CoreValuesThread,
};

impl ApiClient {
    /// POST /api/v2/ai-core-values/stream (SSE)
    ///
        ...
        F: FnMut(&str, &str) -> Result<()>,
    {
        ...
        let response = self
            .post_stream("/api/v2/ai-core-values/stream", req)
            .await?;
    ...
    pub async fn list_core_values_threads(&self) -> Result<Vec<CoreValuesThread>> {
        let resp: ApiResponse<Vec<CoreValuesThread>> =
            self.get("/api/v2/ai-core-values/threads").await?;
        Ok(resp.data)
    }

    /// GET /api/v2/ai-core-values/threads/:threadId/messages
    pub async fn list_core_values_messages(
        &self,
        thread_id: &str,
    ) -> Result<Vec<CoreValuesMessage>> {
        let path = format!(
            "/api/v2/ai-core-values/threads/{}/messages",
            encode_path_segment(thread_id)
        );
        let resp: ApiResponse<Vec<CoreValuesMessage>> = self.get(&path).await?;
        Ok(resp.data)
    }
}
