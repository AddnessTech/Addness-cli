...
pub(super) fn exec_args(
    ...
    // Addness 操作は filesystem Code API（内部runtimeはCLI）に一本化するためMCPを拒否する。
...
/// Addness 操作は filesystem Code API（内部runtimeは `"$ADDNESS_BIN"`）に一本化するため、
/// エージェントに addness MCP サーバのツールを一切使わせない拒否ルール。
/// `mcp__addness` はサーバレベルの
...
mod tests {
    ...
    /// Addness 操作は filesystem Code API 一本化のため、exec 経路で必ず addness MCP を拒否する。
    #[test]
    ...
    fn upstream_probe_claude_cli_flags() {
        ...
        const REQUIRED: &[&str] = &[
            ...
            "--disallowedTools",              // addness MCP 拒否（filesystem Code APIへ一本化）
        ];
